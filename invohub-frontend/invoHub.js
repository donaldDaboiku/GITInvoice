...
const SUPPORT_EMAIL = 'donalddaboiku@gmail.com';
const API_BASE_URL = 'https://api.invohub.com/api';
const PLAN_OPTIONS = [
    {
        code: 'solo_lifetime',
        label: 'One-time',
        name: 'Solo Lifetime',
        price: '$25.42 once',
        meta: '1 staff, local-first, limited feature set, still installable as a PWA.',
        featured: false
    },
    {
        code: 'team_5',
        ...
        name: 'Team 5',
        price: 'Monthly plan',
        meta: 'Up to 5 staff, shared server data, synced workspace, still runs as a PWA.',
        featured: true
    },
    {
        code: 'team_unlimited',
        ...
        name: 'Team Unlimited',
        price: 'Monthly plan',
        meta: 'Unlimited staff, shared server data, admin controls, and PWA install support.',
        featured: false
    }
];
...
const STORAGE_KEYS = {
    ...
    DEVICE_ACTIVATED: 'invoHub_license_activated',
...
function renderPlanCards() {
    ...
    container.innerHTML = PLAN_OPTIONS.map(plan => `
        <div class="plan-card ${plan.featured ? 'is-featured' : ''}">
            <div class="plan-card-label">${plan.label}</div>
...
function renderWorkspacePlanSummary() {
    const container = document.getElementById('workspace-plan-summary');
    if (!container) return;

    container.innerHTML = `
        <div class="workspace-plan-card">
            <strong>Solo Lifetime</strong>
            <span>One owner, device-first data, and no recurring billing. Best for independent use.</span>
        </div>
        <div class="workspace-plan-card">
            <strong>Team 5</strong>
            <span>Five staff seats, server-backed sync, and shared data while keeping the installable PWA experience.</span>
        </div>
        <div class="workspace-plan-card">
            <strong>Team Unlimited</strong>
            <span>Unlimited staff, centralized workspace management, and room for heavier reporting and approvals.</span>
        </div>
    `;
}

function contactTeamSales(planCode) {
    const url = `mailto:${SUPPORT_EMAIL}?subject=${encodeURIComponent(`InvoHub ${planCode} plan inquiry`)}`;
